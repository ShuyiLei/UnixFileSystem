...
public class MyFileSystem implements FileSystem {
    ...
    private DirectBlock getDirectBlock(int fd, MODE mode) {//FIXME!!!
        ...
        if(blockNum <= 9) {
             return simpleGetDirectBlock(inode.ptr, blockNum, mode, blockOff);
        } 
        else if(blockNum <= 9 + IndirectBlock.COUNT){
            IndirectBlock indirectBlock = simpleGetIndirectBlock(inode.ptr, 10, mode);
            ...
            DirectBlock res = simpleGetDirectBlock(indirectBlock.ptr, blockNum - 10, mode, blockOff);
            disk.write(inode.ptr[10], indirectBlock);
            return res;
        }
        else if(blockNum <= 9 + IndirectBlock.COUNT + IndirectBlock.COUNT * IndirectBlock.COUNT) {
        	int remaining = blockNum - ( 9 + IndirectBlock.COUNT + 1);
        	IndirectBlock indirBlockLayer0 = simpleGetIndirectBlock(inode.ptr, 11, mode);
            if(indirBlockLayer0 == null) {
            	if(mode == MODE.r) {
            		return DirectBlock.hole;
            	}
            	else {
            ...
            int index1 = remaining / IndirectBlock.COUNT;
            int index2 = remaining % IndirectBlock.COUNT;
            IndirectBlock indirBlockLayer1 = simpleGetIndirectBlock(indirBlockLayer0.ptr, index1, mode);
            if(indirBlockLayer1 == null) {
            	if(mode == MODE.r) {
            		return DirectBlock.hole;
            	}
            	else {
            ...
            DirectBlock res = simpleGetDirectBlock(indirBlockLayer1.ptr, index2, mode, blockOff);
            disk.write(indirBlockLayer0.ptr[index1], indirBlockLayer1);
            disk.write(inode.ptr[11], indirBlockLayer0);
            return res;
        }
        else if(blockNum <= 9 + IndirectBlock.COUNT + IndirectBlock.COUNT * IndirectBlock.COUNT 
        		+ IndirectBlock.COUNT * IndirectBlock.COUNT * IndirectBlock.COUNT) {
        	int remaining = blockNum - (9 + IndirectBlock.COUNT + IndirectBlock.COUNT * IndirectBlock.COUNT + 1);
        	int index1 = remaining / (IndirectBlock.COUNT * IndirectBlock.COUNT);
        	int index2 = (remaining / IndirectBlock.COUNT) % IndirectBlock.COUNT;
        	int index3 = remaining % IndirectBlock.COUNT;
        	IndirectBlock indirBlockLayer0 = simpleGetIndirectBlock(inode.ptr, 12, mode);
            if(indirBlockLayer0 == null) {
            	if(mode == MODE.r) {
            		return DirectBlock.hole;
            	}
            ...
            IndirectBlock indirBlockLayer2 = simpleGetIndirectBlock(indirBlockLayer1.ptr, index2, mode);
            if(indirBlockLayer2 == null) {
            	if(mode == MODE.r) {
            		return DirectBlock.hole;
            	}
            	else {
            ...
            DirectBlock res = simpleGetDirectBlock(indirBlockLayer2.ptr, index3, mode, blockOff);
            disk.write(indirBlockLayer1.ptr[index2], indirBlockLayer2);
            disk.write(indirBlockLayer0.ptr[index1], indirBlockLayer1);
            disk.write(inode.ptr[12], indirBlockLayer0);
            return res;
        }
        else {
		...
		throw new RuntimeException("This won't be reached");
...
//        return new DirectBlock(disk, inode.ptr[blockNum], blockOff, fresh);
    ...
    // This method get the direct block from ptr[], either inode's ptr[] or an
    // indirectblock's ptr[], and a index for the ptr[]
    // This only go through one layer ( no intermediate Indirect block,
    // So it is "simple"
    private DirectBlock simpleGetDirectBlock(int[] ptr, int blockNum, MODE mode,
                                             int blockOff){
        boolean fresh = ptr[blockNum] == 0;
        if(fresh)
            if(mode == MODE.r)
                return DirectBlock.hole;
            else if((ptr[blockNum] = freeMap.find()) == 0)
                return null;
        return new DirectBlock(disk, ptr[blockNum], blockOff, fresh);

    }
    
    // This also only go through one layer ( no intermediate Indirect block
    private IndirectBlock simpleGetIndirectBlock(int[] ptr, int blockNum, MODE mode){
        boolean fresh = ptr[blockNum] == 0;
        IndirectBlock indirectBlock = new IndirectBlock();
        if(fresh) {
        	if(mode == MODE.w && (ptr[blockNum] = freeMap.find()) != 0) {
        		disk.write(ptr[blockNum], indirectBlock);
        	}
        	else {
        		return null;
        	}
        	...
        	disk.read(ptr[blockNum], indirectBlock);
        }
